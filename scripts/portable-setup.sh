...
if [[ -d "$REPO_DIR/.git" ]]; then
    ...
    if command -v git &>/dev/null; then
        git clone --depth 1 https://github.com/hitechcloud-vietnam/nvHive.git "$REPO_DIR" 2>/dev/null || {
        ...
        mkdir -p "$REPO_DIR"
        curl -sSL https://github.com/hitechcloud-vietnam/nvHive/archive/refs/heads/main.tar.gz | \
