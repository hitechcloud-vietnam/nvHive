...
#
# Install:
#   curl -sSL https://raw.githubusercontent.com/hitechcloud-vietnam/nvHive/main/install-mac.sh | bash
#
# What lives in ~/nvh/:
...
if command -v git &>/dev/null; then
    git clone --depth 1 -q https://github.com/hitechcloud-vietnam/nvHive.git "$NVH_REPO" 2>/dev/null || {
        mkdir -p "$NVH_REPO"
        curl -sSL https://github.com/hitechcloud-vietnam/nvHive/archive/refs/heads/main.tar.gz | tar xz -C "$NVH_REPO" --strip-components=1
    }
else
    mkdir -p "$NVH_REPO"
    curl -sSL https://github.com/hitechcloud-vietnam/nvHive/archive/refs/heads/main.tar.gz | tar xz -C "$NVH_REPO" --strip-components=1
